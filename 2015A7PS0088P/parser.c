	...
	do{
		if(isEndSymbol(symStack->symbol) || symStack->symbol->isTerminal){
			...
			if(ptable[rowIx][colIx].symbol==NULL){
				...
				panicMode=1;
				isSyntacticallyCorrect = 0;
				mergeList(&synchronizationSet,symStack->symbol->follow,0);
				insertSymbol(&synchronizationSet,getSymbolIndex(&symbols,"SEMICOLON"));

				while(checkInList(synchronizationSet,getSymbolIndex(&symbols,getTokenFromId(tokenLex.id,tokenLex.name)))==0){
					tokenLex = getToken(fp,&fileBuff,lexeme,&begin);
					begin++;
				}
				pop(&symStack);
